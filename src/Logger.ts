...
export default class Loggerd {
    ...
    public format = winston.format.combine(
        winston.format.printf(info => {
            const levelColor = this.levelColors[info.level] || ('green' as unknown as keyof typeof chalk);

            const timestamp = chalk.gray(dayjs().format('YYYY-MM-DD HH:mm:ss'));
            const level = (chalk as unknown as any)?.[levelColor](`[${info.level.toUpperCase()}]`);
